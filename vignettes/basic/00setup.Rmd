...
if (!exists("kable_defined")) {
  ...
  if (!exists("first_setup_run")) {
    first_setup_run <- FALSE
  }

  print.data.frame <- function(x, ...) print(knitr::kable(x, type))
  print.data.table <- function(x, ...) print(knitr::kable(x, type))

  print.icesat2.atl03_seg_dt <- function(x, ...) print(knitr::kable(x, type))
  print.icesat2.atl08_dt <- function(x, ...) print(knitr::kable(x, type))
  print.icesat2.atl03_dt <- function(x, ...) print(knitr::kable(x, type))
  print.icesat2.atl03atl08_dt <- function(x, ...) print(knitr::kable(x, type))
}
...
if (!exists("atl03_h5")) {
  ...
  atl03_atl08_ls <- lapply(seq_along(atl03_h5), function(ii) {
    dt <- ATL03_ATL08_photons_attributes_dt_join(atl03_h5[[ii]], atl08_h5[[ii]])
    dt[["rgt"]] <- atl03_h5[[ii]][["orbit_info/rgt"]][]
    dt
  })
