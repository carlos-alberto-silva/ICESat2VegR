...
ICESat2VegR::ee_initialize('ee-caiohamamura')
outdir <- normalizePath("~/ic2_workshop_data/")
```
...
```{r export_atl03_to_vector, eval=TRUE}
library(terra)
...
raster_max_h_canopy <- mapview::mapView(
  max_h_canopy,
  map = map_vect,
  col.regions = redYellowGreen
)

raster_max_h_canopy@map
```


Multiple attributes:

```{r raster_multiple_attributes}
...
```{r maps-2x2, fig.show='hold', out.width='49%', echo = FALSE}
mapview::mapshot2(m1@map, file="readme/m1.png", vheight=540, vwidth = 540)
mapview::mapshot2(m2@map, file="readme/m2.png", vheight=540, vwidth = 540)
mapview::mapshot2(m3@map, file="readme/m3.png", vheight=540, vwidth = 540)
mapview::mapshot2(m4@map, file="readme/m4.png", vheight=540, vwidth = 540)
knitr::include_graphics(c("readme/m1.png","readme/m2.png","readme/m3.png","readme/m4.png"))
```
...
Visualize clipped bounding box

```{r visualize_clipped_bbox}
...
Now visualize clip by polygon `id`

```{r clip_atl08_attributes_geometry}
aoi <- file.path(outdir, "example_aoi.gpkg")
aoi_vect <- terra::vect(aoi)
...
```{r visualize_atl08_upscaling_input}
library(terra)
library(leaflet)
...
hls_search <- search_datasets("Harmonized", "Landsat", "TOA")
hls_search
```
...
```{r visualize_hls_masked, echo = FALSE}
map |> leaflet::hideGroup("unmasked") |> leaflet::showGroup("masked")
```
