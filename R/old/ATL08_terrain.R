      ...
      for (col in terrain_attribute) {
        ...
        if (atl08_h5v2_i$exists(base_addr) && atl08_h5v2_i$exists(metric_address))
          if (metric_address %in% c("h_te_best_fit_20m","subset_te_flag")) {
            ...
            if (col=="subset_te_flag") { colnames(m)[(ncol(m)-4):ncol(m)]<-paste0("subset_te_flag_geo_",1:5)}
            if (col=="h_te_best_fit_20m") { colnames(m)[(ncol(m)-4):ncol(m)]<-paste0("h_te_best_fit_20m_",1:5)}
