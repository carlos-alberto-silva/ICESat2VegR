...
#'
#' @usage ATL08_canopy_segStat(atl03_atl08_dt, func, seg_length,ph_class,beam,quality_ph,night_flag)
#'
...
#'top_canopy <-ATL08_canopy_segStat(atl03_atl08_dt, func=max(ph_h),
#'                                seg_length = 30,
#'                                ph_class=c(2,3),
...
#' @export
ATL08_terrain_dt_polyStat <- function(atl08_canopy_dt, func, id=NULL) {
  ...
  if (is.null(id)) {
    metrics <- lazy_apply_dt_call(dt = atl08_canopy_dt2, call = call)
    metrics <- as.data.table(metrics)
    if (ncol(metrics) < 2) {
      colnames(metrics) <- paste0(call)[1]
    }
  } else {
    metrics <- lazy_apply_dt_call(dt = atl08_canopy_dt2, call = call, group.by = paste0("by = ", id))
