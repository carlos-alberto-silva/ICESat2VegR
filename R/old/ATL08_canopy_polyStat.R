...
#' @export
ATL08_canopy_dt_polyStat <- function(atl08_canopy_dt, func, id=NULL) {
  ...
  if (is.null(bin)) {
    metrics <- lazy_apply_dt_call(dt = atl08_canopy_dt, call = call)
    metrics <- as.data.table(metrics)
    if (ncol(metrics) < 2) {
      colnames(metrics) <- paste0(call)[1]
    }
  } else {
    metrics <- lazy_apply_dt_call(dt = atl08_canopy_dt, call = call, group.by = paste0("by = ", id))
