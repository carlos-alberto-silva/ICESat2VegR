...
#'
#'@usage ATL08_canopy_attributes(atl08_h5, beam)
#'
...
#'canopy_metrics<-ATL08_canopy_attributes(atl08_h5=atl08_h5)
#'head(canopy_metrics)
#'canopy_metrics0
#'close(atl08_h5)
#'@export
ATL08_canopy_attributes <- function(atl08_h5,
          ...
          for (col in canopy_attribute) {
            ...
            if (atl08_h5v2_i$exists(base_addr) && atl08_h5v2_i$exists(metric_address))
              if (metric_address %in% c("h_canopy_20m","canopy_h_metrics_abs","subset_can_flag","canopy_h_metrics")) {
                ...
                if (col=="h_canopy_20m") { colnames(m)[(ncol(m)-4):ncol(m)]<-paste0("h_canopy_20m_geo_",1:5)}
                if (col=="canopy_h_metrics_abs") { colnames(m)[(ncol(m)-17):ncol(m)]<-paste0("AH",seq(10,95,5))}
                if (col=="subset_can_flag") { colnames(m)[(ncol(m)-4):ncol(m)]<-paste0("subset_can_flag_geo_",1:5)}
                if (col=="canopy_h_metrics") { colnames(m)[(ncol(m)-17):ncol(m)]<-paste0("RH",seq(10,95,5))}
              } else {
