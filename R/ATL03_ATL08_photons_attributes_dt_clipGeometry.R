...
#' @param clip_obj An object of class [`terra::SpatVector`], which can be loaded as an ESRI shapefile using [terra::vect] function.
#' @param split_by Optional. clip_obj ID. If defined, the data will be clipped by each clip_obj using
...
ATL03_ATL08_photons_attributes_dt_clipGeometry <- function(atl03_atl08_dt, clip_obj, split_by = NULL) {
  ...
  # Clip data by bounding box of the clip_obj
  ...
  if (nrow(atl03_atl08_dt) == 0) {
    warning("The clip_obj does not overlap the ATL08 data")
    empty <- data.table()
